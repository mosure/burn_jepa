...
state/update regularization, Memory-ALiBi carried state, and a small
norm-only continuation closes most of the sparse image-token gap to a dense
image baseline when distilling to the 3D video teacher: on a matched slice,
base sparse image tokens score $0.4631/0.8048$ loss/cosine, dense image tokens
score $0.3288/0.8636$, and sparse SC-TTT scores $0.3410/0.8576$.  The current
stage2 checkpoint remains stable in explicit long-run stress gates:
$0.2794/0.8851$ over a 1088-window same-stream repeat with late--early loss
$-0.0009$, and $0.2573/0.8924$ over a 512-window adversarial stitch with
late--early loss $-0.0193$.  We separately implement an
In-Place-TTT-inspired MLP path and a stricter conformance lane, but the shipped
result remains the SC-TTT adapter/Memory-ALiBi model because matched free-run
strict in-place eval is stable but substantially lower quality.  The main
remaining gates are larger open-set video, repeated measurements, downstream
baselines, exact in-place reference parity, and lower-cost backward/optimizer
kernels.
\end{abstract}
...
Let $W_i\in\mathbb{R}^{B\times C\times T\times H\times W}$ be the current
rolling teacher window from a stream, with $T=16$ frames in the reported runs,
and let $G$ be the V-JEPA tubelet grid.  The window contains only the current
teacher target horizon; earlier context must reach the student through carried
state rather than by extending the teacher sample with future or privileged
frames.  A sparse context mask $M_i\subseteq G$ selects the tokens refreshed for
that window.  Given a frozen dense V-JEPA teacher $E_\theta$, learn an
encoder-only student $S_\psi$ with recurrent TTT state $F_i$ such that
\begin{equation}
  S_\psi(W_i,M_i,F_{i-1}) \approx E_\theta(W_i)
\end{equation}
...
\item Teacher supervision is a rolling 16-frame V-JEPA video target in the
  production configs.  Longer temporal context is supplied by carried TTT state,
  not by giving the teacher future frames inside one target sample.
\item Long-running streams use the trained state dynamics and diagnostic
  reset/decay policies.  Simple unbounded carried state was a measured failure
  in earlier stress gates; the current Memory-ALiBi continuation is evaluated
  with much longer same-stream and adversarial-stitch gates, but still should
  not be read as a broad arbitrary-distribution stability guarantee.
...
is carried with dynamics that were not trained or measured, fast weights can
decohere.  If scene switches are stitched without state dynamics or reset
coverage that matches training, the state can contaminate the next scene.  If
...
\subsection{In-Place TTT Conformance Lane}

In-Place TTT \cite{feng2026inplace} motivates a different integration point:
selected pretrained MLP down-projections become the base fast weights rather
than adding standalone residual adapters.  We implement this as an ablation,
not as the default SC-TTT model.  The non-strict \texttt{in\_place\_mlp} lane
reuses selected V-JEPA MLP down-projections but keeps older local update
semantics.  The stricter \texttt{in\_place\_mlp\_strict} lane is the
conformance-oriented path: it uses the selected MLP \texttt{fc2} as the
fast-weight base, uses causal target generation, applies the current fast
weight before updating it, and updates only on full chunks.  It intentionally
does not use Memory-ALiBi, because multi-bank state is a local SC-TTT extension
rather than the single fast-weight cache used by the In-Place TTT formulation.

The paper's empirical claims therefore stay separated.  Tables and deployment
results labeled SC-TTT use the residual adapter/Memory-ALiBi path.  The strict
in-place lane is currently a conformance and performance ablation.  A matched
free-run run with rolling 16-frame teacher windows has now been completed, but
the lane remains far below the adapter baseline and still lacks exact
reference-step parity; it is not the promoted checkpoint.
...
\item Run the frozen dense teacher on the current rolling 16-frame video window
  and detach the teacher features.
...
at 256px with 16-frame student clips and a frozen 3D teacher.  The production
configs set \texttt{dataset.frames=16} and
\texttt{training.teacher\_window\_frames=16}; this makes each teacher target a
rolling current-window target rather than an expanding or future-looking
teacher sample.  The verified SC-TTT run uses sparse context masks with 410 of
2048 tokens (20.0\%) and 103 target tokens (5.0\%).
...
Image-to-video controls are required for fair interpretation.  A sparse
non-TTT baseline must be trained or measured against the same 3D teacher target;
otherwise the comparison silently assumes that pretrained image-token features
and video-tubelet features already live in the same distribution.  We therefore
report dense image, sparse image, SC-TTT, no-TTT norm-only, no-TTT last-block,
and TTT continuation probes separately where available.
...
\begin{table}[t]
\centering
\caption{Verified checkpoints and continuations.  The current active candidate
is the norm-only stage2 continuation from the Memory-ALiBi checkpoint; the
earlier SIGReg and long-stable rows are retained as historical ablations.}
\label{tab:verified}
\begin{tabular}{lrrrr}
\toprule
...
SC-TTT Memory-ALiBi + norms & 512 & 1024 & 0.1649 & 0.2549 \\
...
long-rollout eval worsened.  The stage2-norms continuation starts from the
Memory-ALiBi candidate, keeps sparse patchify frozen, adds only 43,008
pretrained LayerNorm trainable parameters, and is the current promoted
checkpoint in this draft.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Image-student to video-teacher alignment controls.  The sparse
SC-TTT row uses the deployed recurrent path; dense and sparse image rows are
base V-JEPA 2.1 image-token baselines against the same rolling 3D teacher
target.  Lower loss and higher cosine are better.}
\label{tab:image-video-alignment}
\begin{tabular}{lrrr}
\toprule
Lane & Student tokens & Loss & Cosine \\
\midrule
Dense image V-JEPA 2.1 baseline & 2048 / 2048 & 0.3288 & 0.8636 \\
Sparse image V-JEPA 2.1 baseline & 410 / 2048 & 0.4631 & 0.8048 \\
Sparse Memory-ALiBi SC-TTT & 410 / 2048 & 0.3410 & 0.8576 \\
\bottomrule
\end{tabular}
\end{table}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Stage2 trainable-capacity probes.  No-TTT controls train the image
pathway against the same video-teacher feature loss, so TTT is not compared
only against an unfine-tuned image baseline.}
\label{tab:stage2-probes}
\begin{tabular}{lrrrrr}
\toprule
Lane & Steps & Dense/sparse steps & Trainable params & Pre loss/cos & Post loss/cos \\
\midrule
No-TTT image-to-video, norms & 96 & 65 / 31 & 43K & 0.3981 / 0.8293 & 0.3954 / 0.8304 \\
No-TTT image-to-video, last 2 blocks & 96 & 65 / 31 & 14.18M & 0.3981 / 0.8293 & 0.3967 / 0.8299 \\
TTT + norms, from Memory-ALiBi best & 96 & 65 / 31 & 3.59M & 0.3557 / 0.8477 & 0.3493 / 0.8504 \\
TTT + norms, from Memory-ALiBi best & 192 & 66 / 126 & 3.59M & 0.3557 / 0.8477 & 0.3465 / 0.8516 \\
\bottomrule
\end{tabular}
\end{table}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Matched free-run CUDA ablation for the strict In-Place TTT lane.  All
rows use rolling 16-frame teacher windows, sparse 410/2048 context tokens, and
deployable self-hidden updates rather than teacher-forced memory targets.}
\label{tab:inplace-smoke}
\begin{tabular}{llrrrr}
\toprule
Lane & Eval & Loss & Cosine & Samples/s & Late--early loss \\
\midrule
SC-TTT adapter stage2 & held-out 164 & 0.2734 & 0.8852 & 3.25 & +0.0076 \\
Strict in-place best & held-out 164 & 0.4273 & 0.8187 & 3.53 & +0.0405 \\
Strict in-place high update & held-out 164 & 0.3920 & 0.8325 & 3.82 & -0.0014 \\
SC-TTT adapter stage2 & cactus 1088 & 0.2794 & 0.8851 & 5.80 & -0.0009 \\
Strict in-place best & cactus 1088 & 0.4799 & 0.7989 & 6.32 & 0.0000 \\
SC-TTT adapter stage2 & stitched 512 & 0.2573 & 0.8924 & 5.20 & -0.0193 \\
Strict in-place best & stitched 512 & 0.4284 & 0.8183 & 5.77 & -0.0212 \\
\bottomrule
\end{tabular}
\end{table}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Current long-rollout promotion gates after the norm-only continuation.
The same-stream gate repeats one held-out stream for 1088 windows; the
adversarial stitch alternates scene boundaries for 512 windows.  These are
stress gates, not a replacement for broader open-set video evaluation.}
\label{tab:stage2-long}
\begin{tabular}{lrrrrr}
\toprule
Lane & Windows & Runtime resets & Loss & Cosine & Late--early loss \\
\midrule
Stage1 Memory-ALiBi final, same-stream repeat & 1088 & 1 & 0.3273 & 0.8646 & -0.0009 \\
Stage2 norms final, same-stream repeat & 1088 & 1 & 0.2794 & 0.8851 & -0.0009 \\
Stage1 Memory-ALiBi final, adversarial stitch & 512 & 1 & 0.2850 & 0.8807 & -0.0229 \\
Stage2 norms final, adversarial stitch & 512 & 1 & 0.2573 & 0.8924 & -0.0193 \\
\bottomrule
\end{tabular}
\end{table}
...
Tables~\ref{tab:coherence}--\ref{tab:stage2-long} are the current long-rollout
...
central limitation and the central fix: simple unbounded carried state fails,
while explicit state dynamics and reset policies matter.  The later
Memory-ALiBi + norms continuation improves both the 1088-window same-stream
gate and the 512-window adversarial-stitch gate, so the draft now treats
stage2-norms as the active candidate.  The claim is still empirical and scoped:
these gates exercise longer carried state than the manifest, but they do not
prove arbitrary-distribution stability.
...
that Gaussian regularization solves long-rollout drift here.  Third, simple
unbounded TTT state carry was a measured failure before the Memory-ALiBi
continuation: the repeated same-stream gate drifted by $+0.1772$ loss over 136
windows, and the stitched-scene gate drifted by $+0.2124$ loss without scene
reset.  Fourth, dense full-grid temporal diagnostics over large token tensors
currently hit a CubeCL CUDA allocator panic; dense primary eval works, but
dense diagnostic probes need chunking or streaming.  Fifth, the 164-window
held-out manifest check improves with persistent state on average but has only
33 contiguous windows in one stream, so it was insufficient by itself.  Sixth,
the strict in-place MLP lane is not promoted: it now passes causal/update smoke
tests and a longer matched free-run CUDA ablation, but it trails the SC-TTT
adapter by a large margin.  The best strict held-out manifest result in this
pass is $0.3920/0.8325$ loss/cosine, versus $0.2734/0.8852$ for the adapter;
the same-stream and adversarial gates likewise stay near sparse-base quality.
It also still lacks reference-step parity against the upstream implementation.
Seventh, patch-diff masks can miss slow semantic changes; refresh policies such
as age-priority, blue-noise refresh, or keyframes are required for persistent
feature caches.
...
Theoretical originality & Benchmark where method can lose & yes & Low-LR continuation, strict in-place smoke, and dense diagnostic failure. \\
Theoretical originality & Failure case & yes & Section 8. \\
Novel algorithm & Baseline & yes & Base sparse V-JEPA 2.1, dense/sparse image-to-video, reset, freeze, reverse/shuffle, dense, low-LR continuation, zero/no-op insertion. \\
Novel algorithm & Measurable improvement & yes & SC-TTT sparse beats base sparse V-JEPA by 0.1064 loss on the full manifest and improves the stage2 1088-window gate over stage1 by 0.0479 loss. \\
Novel algorithm & Ablation & yes & State reset, freeze, order, mask, density, backend, continuation, latent regularizer, in-place vs adapter. \\
...
Novel algorithm & Case where it does not work & yes & Low-LR continuation, dense diagnostic allocator pressure, simple unbounded state carry, unreset scene stitching, and strict in-place not yet winning. \\
...
manifest.  The stage2 Memory-ALiBi + norms continuation improves the long
same-stream and adversarial-stitch gates further.  It fails when state dynamics
do not match the deployment stream, when train-loss-only continuation worsens
carried eval, when the strict in-place MLP lane is used before it beats the
adapter baseline, when the latent Gaussian regularizer is only neutral, when
diagnostic dense probes exceed backend memory behavior, and when masks miss
semantic updates.  Those failures violate the assumptions that eval selection
matches deployment, diagnostics are chunked, sparse masks cover semantic
changes, and the measured state dynamics are active.  Future work should scale
data, add external and downstream baselines, repeat measurements, add an exact
In-Place TTT reference fixture, chunk dense diagnostics, and improve sparse TTT
...
target/burn-jepa-production-final-256/
  ...
  stage2-norms-low-lr/ttt-model.mpk
\end{verbatim}
...
\begin{verbatim}
LONG=target/burn-jepa-production-final-256/stage2-norms-low-lr/ttt-model.mpk
cargo run --no-default-features --features cuda,sparse-patchify-cuda \
  --bin burn-jepa -- eval-ttt \
...
Strict in-place conformance smoke:
\begin{verbatim}
cargo run --no-default-features --features cuda,sparse-patchify-cuda \
  --bin burn-jepa -- train-ttt \
  --config configs/production/vjepa21-ttt-stage1-stream-tbptt-inplace-mlp-strict-thirds-cuda.toml
\end{verbatim}
...
The stress gates make the long-run claim more precise: simple unbounded carry
was unstable, while Memory-ALiBi plus the norm-only continuation is stable on
the measured 1088-window same-stream and 512-window adversarial-stitch gates.
The strict In-Place TTT lane now has causal/update conformance checks and a
matched free-run ablation, but it remains an ablation rather than the shipped
model because its memory update is not yet useful under this V-JEPA
image-student to video-teacher objective.  The evidence is not yet sufficient
for theoretical originality,
...
video, repeated statistics, external and downstream baselines, exact In-Place
TTT reference parity, chunked dense diagnostics, and lower-overhead TTT
backward kernels.
