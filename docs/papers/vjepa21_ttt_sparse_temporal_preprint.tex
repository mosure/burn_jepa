...
result remains the SC-TTT adapter/Memory-ALiBi model because the strict lane is
currently only smoke-validated and not quality-promoted.  The main remaining
gates are larger open-set video, repeated measurements, downstream baselines,
and lower-cost backward/optimizer kernels.
\end{abstract}
...
in-place lane is currently a conformance and performance ablation pending a
long matched training run and reference-step parity fixture; it is not the
promoted checkpoint.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Bounded real-checkpoint CUDA conformance smoke for the In-Place TTT
lanes.  Both rows use rolling 16-frame teacher windows and deployable
self-hidden memory updates.  This is a four-step initialization and plumbing
check, not a promotion-quality training result.}
\label{tab:inplace-smoke}
\begin{tabular}{lrrrr}
\toprule
Lane & Insertion & Eval loss & Eval cosine & Samples/s \\
\midrule
Current SC-TTT adapter baseline & adapter & 0.4136 & 0.8261 & 0.0759 \\
Strict in-place MLP conformance & in\_place\_mlp\_strict & 0.4534 & 0.8091 & 0.0854 \\
\bottomrule
\end{tabular}
\end{table}
...
the strict in-place MLP lane is not promoted: it passes causal/update smoke
tests and a real-checkpoint CUDA smoke, but it has not yet beaten the adapter
baseline or passed reference-step parity against the upstream implementation.
...
real-checkpoint smoke run, but it remains an ablation rather than the shipped
model.  The evidence is not yet sufficient for theoretical originality,
