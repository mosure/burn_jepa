...
| Bevy viewer and Pages workflow are wired. | `crates/bevy_jepa/www`, `crates/bevy_jepa`, `README.md`, `web/README.md`, `.github/workflows/deploy-pages.yml` | The viewer builds as a real wasm artifact before Pages upload and displays input, sparse mask, low-res token PCA, and high-res AnyUp PCA panels through the shared `bevy_burn` WebGPU texture path. | Covered |
