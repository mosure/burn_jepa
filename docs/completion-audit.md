...
| CUDA E2E FPS is measured. | `docs/e2e-benchmark-results.md`, `docs/cuda-benchmark.md`, `docs/workflows/cuda-benchmark.yml` | This host has no usable CUDA runtime for Burn/CubeCL: `nvidia-smi -L` can see an RTX PRO 6000, but no `/dev/nvidia*` nodes are visible and a forced CUDA benchmark fails with `CUDA_ERROR_NO_DEVICE`, emitting only a header CSV. The benchmark now expands the CUDA `RecvError` skip into an explicit CUDA worker/runtime diagnostic. A manual self-hosted CUDA benchmark workflow template now fails if the emitted CSV has no data rows; publishing it under `.github/workflows/` requires workflow-file write permission. | Blocked |
