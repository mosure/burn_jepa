...
fn synthetic_video_batch<B: burn::tensor::backend::Backend>(
    ...
    let videos = (0..batch_size)
        .map(|index| synthetic_video::<B>(index, config.in_channels, 4, 32, 32, device))
        .collect::<Vec<_>>();
    Tensor::cat(videos, 0)
}

fn fixed_width_rows(batch_size: usize) -> Vec<Vec<usize>> {
    let patterns = [
        vec![0, 1, 4, 5],
        vec![2, 3, 6, 7],
        vec![0, 2, 4, 6],
    ...
    (0..batch_size)
        .map(|index| patterns[index % patterns.len()].clone())
        .collect()
}
