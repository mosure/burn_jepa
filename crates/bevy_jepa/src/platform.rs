...
pub mod camera {
    ...
    thread_local! {
        ...
        static RECONSTRUCTION_MODEL_PACKAGE: RefCell<Option<WasmModelPackage>> = const { RefCell::new(None) };
    }
    ...
    #[wasm_bindgen]
    pub fn reconstruction_model_package_input(manifest_json: &str, parts: Array) {
        let package = WasmModelPackage {
            manifest_json: manifest_json.to_string(),
            parts: parts
                .iter()
        ...
        RECONSTRUCTION_MODEL_PACKAGE.with(|cell| {
            *cell.borrow_mut() = Some(package);
        });
    }
    ...
    pub fn reconstruction_model_package() -> Option<WasmModelPackage> {
        if let Some(package) = RECONSTRUCTION_MODEL_PACKAGE.with(|cell| cell.borrow().clone()) {
            return Some(package);
        }
        let package = read_window_model_package("__burnJepaReconstructionModelPackage")?;
        RECONSTRUCTION_MODEL_PACKAGE.with(|cell| {
            *cell.borrow_mut() = Some(package.clone());
        });
        Some(package)
