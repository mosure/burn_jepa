...
fn run(args: Args) -> Result<()> {
    ...
    let mut manifest = Manifest {
        summary: ManifestSummary {
            ...
            fps: args.fps,
            pca_update_every: args.pca_update_every.max(1),
...
fn build_gallery_anyup(
    ...
    .with_attention_mode(args.anyup_attention_mode);
    let mut anyup = AnyUp::<GalleryBackend>::new(config, device)?;
    if let Some(path) = &args.anyup_weights {
...
<body>
  <header>
    ...
    <div class="summary">
      ...
      <div class="stat"><strong>{}</strong><span>PCA sample frames</span></div>
    </div>
  </header>
  <main>{}</main>
</body>
...
"#,
        ...
        summary.token_grid,
        summary.token_grid,
        summary.pca_update_every.max(2),
        body,
    );
